...
class Ship:
	...
	def rotate(self, delta):
		# Reduz velocidade de rotação para metade
		self.angle = (self.angle + delta * 0.5) % 360
...
def main():
	...
	class Paddle:
		...
		def draw(self, surface):
			pygame.draw.rect(surface, (66, 135, 245), self.rect)
		def rebound_ball(self, ball):
			# Rebater como breakout: calcula onde bateu
			if self.rect.colliderect(pygame.Rect(ball.x-ball.radius, ball.y-ball.radius, ball.radius*2, ball.radius*2)):
				hit_pos = (ball.x - self.x) / self.width
				min_angle = 150
				max_angle = 30
				angle_deg = min_angle + (max_angle - min_angle) * hit_pos
				speed = math.hypot(ball.speed * math.sin(math.radians(ball.angle)), ball.speed * math.cos(math.radians(ball.angle)))
				ball.angle = angle_deg
				# Rebater para cima se veio de baixo, para baixo se veio de cima
				if ball.y < self.y:
	...
	while running:
		...
		# BRAKE toggle: Q para ship1, / para ship2 (edge detection)
		if ship1_alive and keys[pygame.K_q]:
			...
			ship1_toggle_last = False
		if ship2_alive and keys[pygame.K_SLASH]:
		...
		if ship1_alive:
			if not ship1.dying:
				...
				if ship1.dying_frames < ROTATE_FRAMES:
					...
					ship1_alive = False
		# Player 2 controls (LEFT/RIGHT rotate, / toggle BRAKE, RCTRL atira)
		if ship2_alive:
			if not ship2.dying:
		...
		for ball in balls:
			ball.move()
		# Rebater bolas na raquete central
		for ball in balls:
			paddle.rebound_ball(ball)
		...
		txt2 = font.render(f"Player 2 BRAKE: {'ON' if ship2.brake else 'OFF'} (/, RCTRL atira)", True, (255,200,200))
